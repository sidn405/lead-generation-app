    ...
    try:
        ...
        with st.spinner('Creating checkout session...'):
            session = stripe.checkout.Session.create(
                ...
                mode="payment",
                success_url=f"http://localhost:8501?package_success=true&username={username}&package={package_name}&timestamp={int(time.time())}",
                cancel_url="http://localhost:8501?package_cancelled=true&username={username}",
